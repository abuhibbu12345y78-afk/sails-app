...
begin
  ...
  for v_affected_session in select distinct unnest(v_affected_sessions) loop
    ...
    v_report := format(E'Sales Summary\n\nDate: %s\n\nGross Sales: %s\nNormal Commission: %s\nFull Commission: %s\nTotal Earnings: %s\nNet Collection: %s',
      v_today, v_totals.gross, v_totals.normal, v_totals.full, v_totals.earnings, v_totals.net);

    if exists (select 1 from public.day_closures where business_date = v_today) then
